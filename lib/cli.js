...
import { buildSparePlane, inspectSparePlane, planeSize } from "./spare.js";
...
async function commandSpare(invocation) {
    ...
    const mode = invocation.copy ? 'copy' : 'links';
    const size = mode === 'copy' ? planeSize(source) : undefined;
    process.stderr.write(`dsh-rescue: building a ${mode} spare plane from ${source}\n`);
    if (size !== undefined && !size.truncated)
        process.stderr.write(`dsh-rescue: copying about ${String(Math.round(size.bytes / 1_048_576))} MB\n`);
    const spare = buildSparePlane(source, dest, mode);
    for (const line of spare.skipped.slice(0, 5))
        process.stderr.write(`dsh-rescue: skipped ${line}\n`);
    if (spare.skipped.length > 5)
        process.stderr.write(`dsh-rescue: skipped ${String(spare.skipped.length - 5)} further entries\n`);
