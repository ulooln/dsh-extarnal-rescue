...
import { defaultStateRoot, launchRescue, verifyCommand } from "./launcher.js";
import { dshHome, homePath, packageRootDir, planeCandidates, requireFromPlane } from "./plane.js";
...
export function parseArgs(args) {
    ...
    for (let index = 0; index < args.length; index++) {
        ...
        if (arg === '--timeout') {
            invocation.timeoutMs = Number(value(args, index, arg));
            index++;
            continue;
        }
 ...
 */
async function applyMechanicalFixes(invocation) {
    const report = await runDoctor({
        ...invocation.plane === undefined ? {} : { plane: invocation.plane },
        profile: invocation.profile,
        packageDir: packageRootDir(),
 ...
 */
function patchListParses(invocation, content) {
    try {
