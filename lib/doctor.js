...
import { dshHome, findCheckout, homePath, importFromPlane, isDirectory, packageRootDir, planeCandidates, requireFromPlane, } from "./plane.js";
import { checkRescueCompatibility } from "./compat.js";
 ...
 */
function bundleRoots(profileDir, planeRoot, packageDir) {
    return [
...
function resolvePackageDir(name, roots, anchors) {
 ...
 */
function readBundle(name, roots, anchors) {
...
export async function runDoctor(options = {}) {
    ...
    for (const dir of profileDirs) {
        ...
        const bundleInfos = bundles.map(bundle => readBundle(bundle, bundleRoots(dir, usable?.root, options.packageDir), bundleAnchors(dir, usable?.root, options.packageDir)));
        const layers = [];
