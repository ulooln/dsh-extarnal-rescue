...
import { extractBootSignals } from "./doctor.js";
/** The harness binary inside a plane. */
export function harnessBin(planeRoot) {
...
export function captureCommand(planeRoot, profile, extraArgs) {
 ...
 */
export async function captureBoot(options) {
    ...
    const attempt = await new Promise((resolveAttempt) => {
        ...
        let poll;
        let settle;
        const finish = (exitCode, booted, pid) => {
            if (settled)
                return;
            settled = true;
            clearTimeout(timer);
        ...
        child.on('exit', (code) => { finish(code, code === 0); });
    });
    return attempt;
}
 ...
 */
export function showsBootFailure(text) {
 ...
 */
export function bootOutcome(exitCode, output) {
    if (exitCode === 0)
        return 'up';
    if (exitCode !== null)
        return 'down';
...
const SNAPSHOT_FILES = ['package.json', 'cordis.patch.yml', 'pnpm-workspace.yaml'];
/**
 ...
 */
export function writeIncident(stateRoot, attempt, context) {
...
function pruneIncidents(stateRoot) {
