...
const LEVEL_TAG = { error: 'ERR ', warn: 'WARN', info: 'INFO' };
...
export function renderReport(report) {
    ...
    if (report.targetProfile !== undefined)
        lines.push(`target      profile ${report.targetProfile}`);
    ...
    if (report.model !== undefined)
        lines.push(`model       ${report.model.provider}/${report.model.model} (${report.model.source})`);
    lines.push('');
    ...
    for (const plane of report.planes) {
        ...
        if (!plane.usable)
            lines.push(`           missing: ${plane.missing.join(', ')}`);
    }
    lines.push('');
    ...
    for (const profile of report.profiles) {
        ...
        if (profile.duplicateIds.length > 0)
            lines.push(`    DUPLICATE IDS ${profile.duplicateIds.join(', ')}`);
        if (profile.orphanTargets.length > 0)
            lines.push(`    ORPHAN PATCH  ${profile.orphanTargets.join(', ')}`);
        if (profile.disabledRows.length > 0)
            lines.push(`    disabled rows ${profile.disabledRows.join(', ')}`);
    }
    lines.push('');
    ...
    if (report.boot.state === undefined) {
        ...
        if (report.boot.advice !== undefined)
            lines.push(`  next step   ${report.boot.advice}`);
    }
    else {
    ...
    if (report.incident !== undefined) {
        ...
        if (incident.dir !== undefined)
            lines.push(`  full output ${incident.dir}`);
    }
    lines.push('');
