...
function fail(exit: (code: number) => void, config: Config, error: unknown): void {
...
/** One tool invocation read back from the durable session log. */
 ...
 */
function readToolActivity(session: Session, from: SessionLogOffset): ToolActivity[] {
...
function describeError(error: unknown): string {
  ...
  for (let depth = 0; depth < 6 && current !== undefined && current !== null; depth++) {
    ...
    if (typeof current === 'object' && typeof (current as { message?: unknown }).message === 'string') {
...
/** The turn outcome the runner acts on. */
interface TurnOutcome {
  kind: 'completed' | 'error' | 'other'
  /** The provider or loop error that ended the turn, when there was one. */
  message?: string
}

/** Read the last turn's end reason, including the error that ended it. */
function lastTurnOutcome(session: Session): TurnOutcome {
 ...
 */
function streamActivity(ctx: Context, agent: Agent, config: Config): () => void {
  let open = false
  const close = (): void => {
    if (!open) return
    say(config, '\n')
  ...
  const disposeStream = ctx.on('agent/assistant-stream', ({ agent: subject, frame }) => {
    ...
    if (chunk.type === 'block-start' || chunk.type === 'block-end' || chunk.type === 'tool-call-delta') close()
  })
  const disposeEvents = ctx.on('session/event', (session: Session, event) => {
...
async function turn(agent: Agent, config: Config, message: string, cursor: SessionLogOffset): Promise<TurnOutcome> {
  ...
  for (const activity of failed) say(config, `  FAIL ${activity.name}  ${activity.detail}\n`)
  const outcome = lastTurnOutcome(agent.session)
  if (outcome.kind === 'error') say(config, `\nturn failed — ${outcome.message ?? 'unknown error'}\n`)
  return outcome
}

/** Print the banner the human reads before the first turn. */
function banner(agent: Agent, config: Config, selection: { provider: string; model: string }): void {
  say(config, [
    '',
    '='.repeat(72),
    ' DSH RESCUE AGENT',
...
async function repl(agent: Agent, config: Config, cursor: SessionLogOffset): Promise<number> {
  ...
  try {
    while (true) {
      const line = await new Promise<string | undefined>((resolveLine) => {
        ...
        readline.once('close', () => {
          if (!answered) resolveLine(undefined)
        })
      })
      ...
      if (trimmed === '/help') { say(config, REPL_HELP); continue }
      if (trimmed === '/mission') { say(config, `\n${config.mission}\n`); continue }
      if (trimmed === '/doctor') { say(config, `diagnosis: ${join(config.runDir, 'doctor.txt')}\n`); continue }
      const reason = await turn(agent, config, trimmed, current)
      current = agent.session.seq as SessionLogOffset
      if (reason.kind === 'error') code = 1
    }
  } finally {
    readline.close()
  }
 ...
 * @param config - run configuration holding the transcript path.
 */
async function preflight(ctx: Context, config: Config): Promise<void> {
  const extensions = ctx.get('deepseekLlmApiExtensions') as
    | { prepare(request: { body: unknown; signal: AbortSignal }): Promise<unknown> }
    | undefined
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (exit === undefined) throw new Error('rescue-runner: the launcher must provide appExit before the tree mounts')
  try {
  ...
  void run(ctx, config, exit).catch((error: unknown) => { fail(exit, config, error) })
}
...
async function run(ctx: Context, config: Config, exit: (code: number) => void): Promise<void> {
  ...
  await loader?.await()
  const agents = ctx.get('agents') as { create(options: unknown): Promise<{ agent: Agent }> } | undefined
  ...
  const firstSeq = agent.session.seq as SessionLogOffset
  const stopStream = streamActivity(ctx, agent, config)
  let code = 0
  try {
    if (config.task !== undefined && config.task.trim() !== '') {
      if ((await turn(agent, config, config.task, firstSeq)).kind === 'error') code = 1
    } else {
