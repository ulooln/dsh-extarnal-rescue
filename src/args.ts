...
export function parseArgs(args: readonly string[]): Invocation {
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    if (arg === '--copy') { invocation.copy = true; continue }
    if (arg === '--check') { invocation.check = true; continue }
    ...
    if (arg === '--from') { invocation.from = resolve(value(args, index, arg)); index++; continue }
    if (arg === '--dest') { invocation.dest = resolve(value(args, index, arg)); index++; continue }
    ...
    if (arg === '--timeout') {
      ...
      const ms = Number(raw)
      if (!Number.isFinite(ms) || ms <= 0) throw new Error(`--timeout must be a positive number of milliseconds (got ${raw})`)
    ...
    if (command === undefined) {
      if (!COMMANDS.has(arg)) throw new Error(`unknown command ${arg}; run dsh-rescue --help`)
      command = arg as Invocation['command']
      continue
    }
    rest.push(arg)
  }
