...
import { packageRootDir, planeCandidates, requireFromPlane } from './plane.ts'
 ...
 */
export function patchListParses(plane: string | undefined, content: string): boolean {
  try {
 ...
 */
export async function applyMechanicalFixes(options: RepairOptions): Promise<RepairOutcome> {
  ...
  if (unusable.length > 0) {
    ...
    const dshSection = (parsed.dsh ?? {}) as Record<string, unknown>
    ...
    if (kept.length !== list.length) {
      ...
      let parses = false
      try {
        parses = typeof (JSON.parse(after) as { dsh?: unknown }).dsh === 'object'
      } catch {
        parses = false
      }
      if (!parses) {
