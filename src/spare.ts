...
import { probePlane } from './plane.ts'
 ...
 */
export function buildSparePlane(source: string, dest: string, mode: SpareMode = 'links'): SpareResult {
  const sourceRoot = resolve(source)
  const destRoot = resolve(dest)
  if (sourceRoot === destRoot) throw new Error('the spare plane cannot be the plane it is built from')
  ...
  for (const name of packageEntries(sourceRoot)) {
    try {
      ...
      if (ensureLink(resolvedSource(sourceRoot, name), join(destRoot, name))) entries += 1
    } catch (error) {
...
function resolvedSource(sourceRoot: string, relative: string): string {
 ...
 */
function ensureLink(target: string, path: string): boolean {
...
function packageEntries(root: string): string[] {
  ...
  for (const name of entriesOf(root)) {
    ...
    if (name.startsWith(METADATA_PREFIX)) continue
    if (!name.startsWith('@')) {
      if (hasManifest(root, name)) entries.push(name)
      continue
    }
    for (const child of entriesOf(join(root, name))) {
      if (child.startsWith(METADATA_PREFIX)) continue
      if (hasManifest(root, `${name}/${child}`)) entries.push(`${name}/${child}`)
    }
  }
  return entries
}
...
function hasManifest(root: string, relative: string): boolean {
 ...
 */
export function inspectSparePlane(dest: string): SpareInspection {
  const destRoot = resolve(dest)
...
function listedEntries(root: string): string[] {
  const entries: string[] = []
  for (const name of entriesOf(root)) {
    if (name === '.bin' || name.startsWith(METADATA_PREFIX)) continue
    if (!name.startsWith('@')) {
      entries.push(name)
      continue
    }
 ...
 */
export function planeSize(root: string, limit = 200_000): { bytes: number; truncated: boolean } {
  let bytes = 0
  let seen = 0
  ...
  while (queue.length > 0) {
    ...
    for (const name of names) {
      if (seen >= limit) return { bytes, truncated: true }
