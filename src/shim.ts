 ...
 */
export function writeShim(stateRoot: string): ShimResult {
  try {
    ...
    if (onPath) return { files: [cmd, sh], onPath }
 ...
 */
function launcherAdvice(stateRoot: string, cmd: string): string {
 ...
 */
export function pathNoticeIsNews(stateRoot: string, onPath: boolean): boolean {
