...
function incidentBlock(incident: Incident | undefined): string {
 ...
 */
function readHandoff(harnessHome: string): string | undefined {
 ...
 */
export function buildMission(context: MissionContext, task?: string): string {
  ...
  if (report.boot.state !== undefined) {
    ...
    sections.push([
      ...
      boot.crashed
        ? [
            'A previous harness run did not reach its ready state. This was inferred from the boot handshake rather than captured live, so no operator had to be watching when it died.',
            `started           ${boot.state?.startedAt ?? 'unknown'}${boot.state?.pid === undefined ? '' : ` (pid ${String(boot.state.pid)})`}`,
            `last good boot    ${boot.state?.lastGoodAt ?? 'none recorded'}`,
            `classified as     ${boot.reason ?? 'unknown'}`,
            boot.advice === undefined ? '' : `next step         ${boot.advice}`,
            '',
            'Treat the classified cause as the leading hypothesis, confirm it against the captured output above, and prefer the smallest fix it implies. Anything changed after the last good boot is the first suspect.',
          ].filter(line => line !== '').join('\n')
        : [
            `The last harness run reached its ready state at ${boot.state?.okAt ?? 'unknown'}; the last good boot was ${boot.state?.lastGoodAt ?? 'unknown'}.`,
            'If the deployment is broken now, the change happened after that time.',
          ].join('\n'),
    ].join('\n'))
  }
  ...
  sections.push([
    ...
    '2. Isolate. If the failure names a plugin row or a package, disable exactly that row in the profile\'s `cordis.patch.yml` with `- id: <row-id>` + `disabled: true`, then verify. A row the profile itself inserts is removed by deleting that insert from the same file.',
    '3. A bundle that the launcher cannot mount cannot be disabled — the bundle layer has no disable switch. The repair is to drop its name from `dsh.profile.bundles` in the profile\'s `package.json`, leaving the package installed and the dependency entry in place so it can be reinstalled or repaired later. Never drop the bundle that mounts this rescue package: then the next crash has no tool left.',
    '4. Repair the root cause. Prefer restoring a missing link, correcting a patch id, or removing a duplicated insert over rewriting a composition. Back up each file before editing it.',
    '5. Verify again, and only claim success on a clean boot. Quote the command and its output. If you could not run it, say so and give the edit precisely enough for the human to run it.',
    '6. Report at the end: what failed, what you changed, which files, what verification printed, and anything still at risk. Write that report to the report file named above as well as printing it.',
  ].join('\n'))
