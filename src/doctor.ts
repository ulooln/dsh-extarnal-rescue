...
import type {
  BundleInfo, DoctorReport, Finding, Incident, LinkInfo, PatchLayerInfo, PlaneInfo, ProfileInfo, UnresolvedInsert,
} from './types.ts'
import {
...
function resolvePackageDir(name: string, profileDir: string, planeRoot: string | undefined): string | undefined {
  const candidates = [
...
function readBundle(name: string, profileDir: string, planeRoot: string | undefined): BundleInfo {
  const dir = resolvePackageDir(name, profileDir, planeRoot)
  if (dir === undefined) return { name, resolved: false, error: `package ${name} is not installed in ${profileDir}/node_modules or the plane` }
  const manifest = readJson(join(dir, 'package.json'))
  if (!manifest.ok) return { name, resolved: false, dir, error: manifest.error }
  const dsh = asRecord(asRecord(manifest.value)?.dsh)
  const patch = str(asRecord(dsh?.bundle)?.patch)
  if (patch === undefined) return { name, resolved: false, dir, error: `${name} declares no dsh.bundle.patch, so it contributes no rows` }
  const patchPath = isAbsolute(patch) ? patch : resolve(dir, patch)
  if (!existsSync(patchPath)) return { name, resolved: false, dir, error: `${name} declares ${patch} but that file is missing` }
