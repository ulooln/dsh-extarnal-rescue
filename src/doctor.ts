...
import type {
  BootReport, BootState, BundleInfo, CrashReason, DoctorReport, Finding, Incident, LinkInfo, PatchLayerInfo,
...
/** One parsed YAML or JSON document with its failure kept as data. */
interface Parsed {
  ok: boolean
  value: unknown
  error?: string
...
function readYaml(path: string, planeRoot: string | undefined, schema: unknown): Parsed {
  let text: string
  try {
    text = readFileSync(path, 'utf8')
  } catch (error) {
  ...
  if (planeRoot === undefined) return { ok: false, value: undefined, error: `${path}: no usable plane to parse YAML` }
  const yaml = requireFromPlane(planeRoot, 'js-yaml') as { load(text: string, options?: unknown): unknown } | undefined
  if (yaml === undefined) return { ok: false, value: undefined, error: `${path}: js-yaml unavailable in ${planeRoot}` }
  try {
...
function asRows(value: unknown): Record<string, unknown>[] {
  ...
  return value.filter((row): row is Record<string, unknown> => asRecord(row) !== undefined)
}
...
function readBundle(name: string, anchors: readonly string[]): BundleInfo {
  ...
  if (!manifest.ok) return { name, resolved: false, dir, code: 'manifest-invalid', error: manifest.error }
...
function readLayer(path: string, role: PatchLayerInfo['role'], planeRoot: string | undefined, schema: unknown): PatchLayerInfo {
  const layer: PatchLayerInfo = { path, role, targets: [], inserts: [], insertRows: [], disables: [], problems: [] }
  if (!existsSync(path)) {
    if (role === 'bundle') {
  ...
  for (const [index, row] of asRows(parsed.value).entries()) {
    ...
    if (Array.isArray(insert)) {
      for (const inserted of asRows(insert)) {
        ...
        if (insertedDisabled && insertedId !== undefined) layer.disables.push(insertedId)
      }
    }
    if (id !== undefined) {
      if (row.disabled === true) layer.disables.push(id)
      else if (insert === undefined) layer.targets.push(id)
      else layer.targets.push(id)
...
function readLinks(manifest: Record<string, unknown>, profileDir: string): LinkInfo[] {
  ...
  for (const [name, spec] of Object.entries(dependencies)) {
    ...
    try {
      ...
      if (stat.isSymbolicLink()) pointsTo = resolve(dirname(installedPath), readlinkSync(installedPath))
    } catch {
...
export function extractBootSignals(output: string): string[] {
  const patterns = [
    /fatal load failure/,
...
function resolvesRowName(name: string, layerPath: string, profileDir: string, planeRoot: string | undefined, packageDir: string | undefined): boolean {
  ...
  if (name.startsWith('./') || name.startsWith('../')) return existsSync(resolve(dirname(layerPath), name))
...
export function readIncident(root: string = homePath('rescue')): Incident | undefined {
  const pointer = join(root, 'incidents', 'latest.json')
  if (existsSync(pointer)) {
    const parsed = readJson(pointer)
    if (parsed.ok) {
      const record = asRecord(parsed.value)
      if (record !== undefined && typeof record.output === 'string') return record as unknown as Incident
    }
  }
  ...
  const record = asRecord(parsed.value)
  if (record === undefined || typeof record.output !== 'string') return undefined
  return { ...(record as unknown as Incident), dir: newest }
}

/** The file the boot handshake reads and writes inside the rescue state root. */
export function bootStatePath(stateRoot: string = homePath('rescue')): string {
 ...
 */
export function readBootState(stateRoot: string = homePath('rescue')): BootState | undefined {
  const parsed = readJson(bootStatePath(stateRoot))
  if (!parsed.ok) return undefined
  const record = asRecord(parsed.value)
  if (record === undefined || typeof record.ok !== 'boolean') return undefined
  return record as unknown as BootState
}
 ...
 */
export function writeBootState(stateRoot: string, state: BootState): string | undefined {
  try {
 ...
 */
export function classifyCrash(text: string): CrashReason {
  if (/corrupt Zstandard session log/i.test(text)) return 'session-corrupt'
  if (/declares no dsh\.bundle|cannot resolve profile bundle|failed to read bundle|dsh\.bundle\.patch/i.test(text)) return 'bundle-check'
  if (/EADDRINUSE|address already in use/i.test(text)) return 'port-bind'
  if (/settings\.yaml|is not valid YAML|failed to parse (patches|overlay|config)/i.test(text)) return 'settings'
  if (/already registered|duplicate loader entry|plugin\(s\) failed to load|failed to load plugin|did not activate|cannot find (module|package)|ERR_MODULE_NOT_FOUND/i.test(text)) return 'patch-tree'
  return 'unknown'
}
 ...
 */
export function crashAdvice(reason: CrashReason): string {
  switch (reason) {
 ...
 */
export function readBootReport(stateRoot: string = homePath('rescue')): BootReport {
  const state = readBootState(stateRoot)
  if (state === undefined) return { crashed: false }
  ...
  if (!crashed) return { state, crashed: false }
...
function lastCapturedOutput(stateRoot: string): string | undefined {
  const incident = readIncident(stateRoot)
  if (incident !== undefined && incident.output.trim() !== '') return incident.output
  return safeRead(join(stateRoot, 'boot.log'))
}

/** Read the model route the harness would select, without booting it. */
function readModelRoute(planeRoot: string | undefined): { provider: string; model: string; source: string; catalog?: string[]; error?: string } | undefined {
  ...
  const parser = yaml as { load(text: string): unknown } | undefined
...
function readCredentials(): { deepseekKey: boolean; source: string } {
  if ((process.env.DEEPSEEK_API_KEY ?? '') !== '') return { deepseekKey: true, source: 'environment' }
  const file = homePath('.credentials.yaml')
  if (!existsSync(file)) return { deepseekKey: false, source: 'no environment variable and no .credentials.yaml' }
...
async function loadEntrySchema(planeRoot: string | undefined): Promise<unknown> {
  if (planeRoot === undefined) return undefined
  try {
    const include = await importFromPlane(planeRoot, '@deepseek-ai/cordis-plugin-include', 'lib', 'index.js') as { entryListSchema?: unknown }
...
function portState(port: number): Promise<boolean> {
...
export async function runDoctor(options: DoctorOptions = {}): Promise<DoctorReport> {
  ...
  for (const dir of profileDirs) {
    ...
    if (homeLayer !== undefined) layers.push(homeLayer)

    const insertCounts = new Map<string, number>()
    for (const layer of layers) {
      for (const id of layer.inserts) insertCounts.set(id, (insertCounts.get(id) ?? 0) + 1)
    }
    ...
    const unresolvedInserts: UnresolvedInsert[] = layers
      .flatMap(layer => layer.insertRows
        .filter(row => row.name !== undefined && !row.disabled)
        .filter(row => !resolvesRowName(row.name as string, layer.path, dir, usable?.root, options.packageDir))
        .map(row => ({ id: row.id, name: row.name as string, layer: layer.path, role: layer.role })))
    ...
    for (const layer of layers) {
      for (const problem of layer.problems) findings.push(problem)
    }
...
export function worstLevel(report: DoctorReport): 'error' | 'warn' | 'info' {
  if (report.findings.some(finding => finding.level === 'error')) return 'error'
  if (report.findings.some(finding => finding.level === 'warn')) return 'warn'
  return 'info'
}
 ...
 */
export function hasEntries(path: string): boolean {
  try {
