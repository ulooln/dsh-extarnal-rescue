...
import { verifyMountedOverrides, checkRescueCompatibility } from './compat.ts'
...
async function bootOnPlane(
  ...
  const mounted = verifyMountedOverrides(
    [...(context.get('loader') as { entries(): Array<{ options?: { id?: unknown; config?: unknown } }> } | undefined)?.entries() ?? []],
    { permissionMode: options.permissionMode, approvalPolicy: 'never' },
  )
  for (const line of mounted.cosmetic) process.stderr.write(`dsh-rescue: warning: ${line}\n`)
