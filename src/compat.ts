...
import { loadAppBoot, packageRootDir } from './plane.ts'
...
export const RESCUE_INSERTS = ['cordis-host-runner', 'tool-cordis'] as const
 ...
 */
export const CRITICAL_OVERRIDES: readonly string[] = ['sandbox-policy', 'approval']
...
export function diffComposition(
  baseRowIds: readonly string[],
  rescueTargets: readonly string[],
  rescueInserts: readonly string[],
): CompositionDiff {
...
/** The layout of an app-boot patch entry, as far as this module reads it. */
interface PatchRow {
  id?: unknown
  insert?: unknown
}

/** The slice of the boot glue this module calls. */
interface AppBootCompat {
  loadOverlayPatches(binName: string, file: string): PatchRow[]
  composeEntries(layers: PatchRow[][]): PatchRow[]
}

/** Every row id one patch list provides, whether by insert or by composition. */
function rowIds(rows: readonly PatchRow[]): string[] {
 ...
 */
function resolvesFrom(name: string, planeRoot: string): boolean {
  if (name.startsWith('cordis:') || name.startsWith('.') || name.includes(':')) return true
  const [first = '', second = ''] = name.split('/')
  const packageName = first.startsWith('@') ? `${first}/${second}` : first
  if (existsSync(join(planeRoot, packageName, 'package.json'))) return true
  try {
    const searchPaths = createRequire(join(planeRoot, '..', 'noop.cjs')).resolve.paths(packageName)
    if (searchPaths === null) return false
 ...
 */
export async function checkRescueCompatibility(planeRoot: string, packageDir: string = packageRootDir()): Promise<RescueCompatibility> {
  const basePatch = join(planeRoot, '@deepseek-ai', 'dsh-base', 'cordis.patch.yml')
  const rescueFile = join(packageDir, 'rescue.cordis.yml')
  const empty: RescueCompatibility = {
    planeRoot, basePatch, missingTargets: [], collisions: [], unresolvedRows: [], criticalMissing: [], ok: false,
  }
  if (!existsSync(basePatch) || !existsSync(rescueFile)) return empty

  let appBoot: AppBootCompat
  let base: PatchRow[]
  let rescue: PatchRow[]
  try {
    appBoot = await loadAppBoot(planeRoot) as unknown as AppBootCompat
  ...
  const rescueInserts: string[] = []
  for (const row of rescue) {
    if (typeof row.id === 'string' && !Array.isArray(row.insert)) rescueTargets.push(row.id)
    if (!Array.isArray(row.insert)) continue
    for (const inserted of row.insert) {
      if (typeof (inserted as PatchRow).id === 'string') rescueInserts.push((inserted as PatchRow).id as string)
    }
  }
  ...
  const unresolvedRows = rescue
    ...
    .map(inserted => (inserted as { name?: unknown }).name)
    .filter((name): name is string => typeof name === 'string')
    .filter(name => !resolvesFrom(name, planeRoot))

  let baseVersion: string | undefined
  try {
    const manifest = JSON.parse(readFileSync(join(planeRoot, '@deepseek-ai', 'dsh-base', 'package.json'), 'utf8')) as { version?: unknown }
    if (typeof manifest.version === 'string') baseVersion = manifest.version
  } catch {
 ...
 */
export function verifyMountedOverrides(
  loaderEntries: readonly MountedEntry[],
  expectations: { permissionMode: string; approvalPolicy: string },
): { blocking: string[]; cosmetic: string[] } {
  ...
  const present = (id: string): boolean => loaderEntries.some(entry => entry.options?.id === id)

  /** `undefined` when the value cannot be read, otherwise whether it matches. */
  const matches = (config: Record<string, unknown> | undefined, key: string, expected: string): boolean | undefined => {
  ...
  const check = (id: string, key: string, expected: string, consequence: string): void => {
    if (!present(id)) {
      blocking.push(`${id}: the deployment provides no such row, so ${consequence}`)
      return
    }
    const verdict = matches(configOf(id), key, expected)
    if (verdict === false) blocking.push(`${id}.${key} is ${JSON.stringify(configOf(id)?.[key])}, not ${JSON.stringify(expected)}`)
    else if (verdict === undefined) cosmetic.push(`${id}.${key} could not be read back, so ${consequence} is unconfirmed`)
  }
