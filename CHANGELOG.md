...
- **`dsh-rescue spare`：升级期间的备用平面**。升级会重写救援自己要读的那份 `node_modules`，所以只读「正在被修的那份安装」的救援，可能被它本该守住的同一次升级带走。这个命令在安装还健康时先建第二份可导入的平面，之后用 `--plane` 指它：
  - `dsh-rescue spare` 从第一个可用平面（或 `--from`）建到 `<state-root>/plane`（或 `--dest`）；每个包单独链接，scope 逐个展开，scope 里没有 manifest 的条目不算包；已经是正确链接的条目不动，所以可以反复重建，也可在备用平面正在被使用时重建。
  - `dsh-rescue spare --check` 只报告状态：包数量、`dsh-base` 版本、是否可用、以及**指向已消失目标的条目**（这些条目在源安装被替换或删除后会出现）。
  - `--copy` 改为复制而非链接。两种模式的差别就是这个功能的全部意义：**links（默认）**瞬时完成、不占额外磁盘，挡得住 profile / patch / bundle / 插件层面的损坏，但源包被替换时链接跟着走；**copy** 物理独立，代价是与源平面等量的磁盘。`spare` 的输出每次都写明当前是哪种模式。
  - 新增退出码 `6`：备用平面缺失或不完整。
...
| 验证项 | 结果 |
| --- | --- |
| 在本机真实部署上建备用平面 | ✅ 从 `C:\Users\skyer\.dsh\profiles\node_modules` 建出 277 个包、`dsh-base 0.1.3-alpha.1`、`usable: yes` |
| 从备用平面真的能起救援 | ✅ `repair --plane <state-root>/plane` 完整跑通一轮 agent（exit 0），stderr 点名该平面（origin `explicit`），**没有**回退到别的平面 |
| `spare --check` | ✅ 报告包数量与可用性；源包被删除后正确点出 dangling 条目（逻辑回归里有对应用例） |
| 门禁 | ✅ 114 项逻辑回归（新增 12 项：可用性、scope 展开、无 manifest 条目、幂等重建、dangling 判定、缺失备用平面） |

### 已知限制

- 链接模式的备用平面**不独立于源包**：`npm/pnpm` 把包原地换掉之后，链接读到的就是新内容。要真正的独立就用 `--copy`，并接受磁盘代价。
- 备用平面不随源自动更新：源升级成功后它仍指向同一批路径，内容自然变成新版本；若升级把路径整个换掉（例如换了安装位置），需要重建或用 `--check` 发现 dangling。
- 其余限制同 0.3.3。
...
- **PATH 提示改为「状态变化时才出现」**。提示本身是 0.3.2 加的，但挂在每次挂载上：默认布局里启动器目录本来就不在 PATH 上，于是每次启动都会重复同一段建议，而读者一旦知道就会跳着看。现在这件事被记进 `$DSH_HOME/rescue/.shim-path-state`，只在状态变化时（首次、或从在 PATH 变成不在 PATH）重新提示一次。`dsh-rescue shim` 是人工显式调用，仍然每次都完整说一遍。
- **启动器只有一份实现**。此前挂载时写的文件与 `dsh-rescue shim` 写的文件是两段代码，注释还不一样（一次挂载就会被覆盖成另一份）。现在两者都调用 `writeShim`，写出同样的字节。
- **修掉 `src/cli.ts` 里的四处乱码**：模块 JSDoc 与启动器注释里的破折号在某次文本往返中被写成了 `鈥?`。功能无影响，但它是脏字节，随 0.3.2 一起发出去过。

### 实测结论

| 验证项 | 结果 |
| --- | --- |
| 门禁 | ✅ 102 项逻辑回归（新增 10 项：启动器内容、`onPath` 判定、提示「首次是新闻 / 同状态不是 / 翻转又是」，以及 PATH 命中时不给建议） |
| 乱码扫描 | ✅ 全仓库 `src/`、`tools/`、根文档再扫一遍，除已修的四处外无残留 |

### 已知限制

- 提示状态的记忆放在状态根里；删掉该目录会让提示再出现一次，这是有意为之（没有记录就无法判断是不是新闻）。
- 其余限制同 0.3.2。
